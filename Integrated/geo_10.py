...
import servo_control as SRV_CTL
import base_filter as BS_FIL
from tracker_logger import (log_open, log_row, log_close,create_raw_logger, set_logging_config)
...
def main():
    ...
    C.AZ_GEAR_RATIO, C.EL_GEAR_RATIO, C.PULSE_MIN_US, C.PULSE_MAX_US
    ...
    if args.mode == "sim":
        GPS.mav_drone = GPS.connect_mav(C.SIM_C.DRONE_ENDPOINT, C.SIM_DRONE_BAUD, True)
        GPS.start_reader(mav_drone, drone_buf, on_raw=raw_logger)
        print(f"[CFG] SIM base @ {C.base_static['lat']}, {C.base_static['lon']}, {C.base_static['alt']}m")
    else:
        mav_drone = GPS.connect_mav(C.DRONE_ENDPOINT, C.DRONE_BAUD, True)
        mav_base  = GPS.connect_mav(C.BASE_ENDPOINT, C.BASE_BAUD,  False)

        GPS.start_reader(mav_drone, drone_buf, on_raw=raw_logger)
        GPS.start_reader(mav_base,  base_buf,  on_raw=raw_logger)
    ...
    def get_zero_snapshot():
        ...
        while time.time() < t_end:
            d = drone_buf.latest()
            if args.mode == "sim":
                b = GPS.GPSSample(time.time(), C.base_static["lat"], C.base_static["lon"], C.base_static["alt"])
            else:
                b = base_buf.latest()
            if d and b:
                return d, b
    ...
    if args.park_face_drone_start and d0 and b0:
        ...
        if info_init:
            ...
            SRV_CTL.smooth_park(pi, s_az0, s_el0, duration_s=args.park_duration, rate_hz=args.park_rate_hz)
    else:
    ...
    if args.mode == "ground":
        if args.base_mode == "static":
            ...
            bs = BS_FIL.stabilize_base(base_buf, window_sec=args.static_window_sec, sd_thresh_m=args.static_sd_thresh_m)
            if bs:
                base_state = bs
    ...
    try:
        ...
        try:
            if args.park_face_drone_exit:
                ...
                SRV_CTL.smooth_park(pi, s_azH, s_elH, duration_s=args.park_duration, rate_hz=args.park_rate_hz)
            else:
