# geo_15.py — zero-filter tracker with static/dynamic base modes and manual calibration + UI
...
from telemetry_payload import TelemetryState, build_tracker_payload
state = TelemetryState()

def start_web_ui():
    """Start web UI in background thread"""
    uvicorn.run(webui.app, host="0.0.0.0", port=8000, log_level="warning")
...
STEP_DEG = 20.0 # Step for Manual Calibration (Add twice of how much you want to see in physical movement)
...
#-----------Starting the UI ----------
    ...
    ui_thread.start()
    print("[INFO] Web UI started on http://localhost:8000")
    ...
    try:
        while True:
            ...
            state.set_base_meta(mode=base_mode_str, locked=base_locked)
            ...
            world_el = abs_el
            #---------------- UI stuff ----------
            state.set_angles(world_az, world_el, used_flip)
            ...
            #------------------ UI Stuff ----------
            state.set_servo(curr_phys_az, curr_phys_el, s_az, s_el, us_az, us_el)
