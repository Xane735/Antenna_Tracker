...
def test_servo_angle_logic():
    """Test the tracking calculations with sample data"""
    # Base location (Bangalore coordinates)
    base_lat = 12.9716   
    base_lon = 77.5946
    ...
    print("=== Antenna Tracker Test Results (using geopy) ===")
    print(f"Base Station: Lat={base_lat}, Lon={base_lon}, Alt={base_alt}m")
    print("-" * 80)
    ...
    for target in targets:
        ...
        if info:
            print(f"\nTarget: {target['name']}")
            print(f"  Coordinates: Lat={target['lat']}, Lon={target['lon']}, Alt={target['alt']}m")
            print(f"  Raw Azimuth: {info['azimuth']}°, Raw Elevation: {info['elevation']}°")
            print(f"  Servo Angles: Az={info['adjusted_azimuth']}°, El={info['adjusted_elevation']}°")
            print(f"  Distance: {info['horizontal_distance']}m, Slant Range: {info['slant_range']}m")
            print(f"  Altitude Diff: {info['altitude_difference']}m")
